...
export function initOpCooler() {
     if (!state.currentModelConfig) return;
            ...
            const kpiRow = document.getElementById('op-kpi-row-cooler');
                    ...
                    kpiRow.innerHTML += `
                        <div class="flex-1 bg-[#1a3842] border border-gray-100 p-2 flex flex-col justify-between rounded min-w-[120px] max-h-[50px]">
                            ...
                            <div class="flex items-baseline gap-1 mt-1">
                                <span class="text-lg font-black text-white font-mono leading-none" id="op3-col-kpi-${tagInfo.tag.replace(/[^a-zA-Z0-9]/g, '')}">---</span>
            ...
            const getTrendBtn = (tag) => `
                <button onclick="Actions.toggleOpTrendCooler('${tag}')" id="op-trend-btn-cooler-${tag.replace(/[^a-zA-Z0-9]/g, '')}" class="text-gray-500 hover:text-[#ebf552] transition-colors focus:outline-none shrink-0">
            ...
            const tCooler = document.getElementById('op-table-coolersec');
            ...
            const liveSensorsList = document.getElementById('op-live-sensors-list-cooler');
            if (liveSensorsList) {
                ...
                filtered.forEach(tag => {
                    const safeId = tag.replace(/[^a-zA-Z0-9]/g, '');
                    const rowId = `op-live-row-cooler-${safeId}`;
                    const valId = `op-live-val-cooler-${safeId}`;
