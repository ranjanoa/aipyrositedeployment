...
export function drawOpParallelChart() {
if (!document.getElementById('op-dash-plotly')) return;
            ...
            let varsToPlot = state.opActiveTrends.length >= 2 ? state.opActiveTrends : (window.currentAIVars || []);
            if (!varsToPlot.length) return;

            const pDims = varsToPlot.slice(0, 5).map(k => {
                const liveVals = state.opHistoryData[k] || [];
                const curr = liveVals.length ? liveVals[liveVals.length - 1].val : 0;

                let goal = curr;
                if (state.opPredictionData[k] && state.opPredictionData[k].length > 0) {
                    goal = state.opPredictionData[k][state.opPredictionData[k].length - 1]; // End of prediction
                } else {
