...
export function drawOpParallelChartKiln() {
if (!document.getElementById('op-dash-plotly-kiln')) return;
            if (document.getElementById('op-dash-chart-parallel-kiln').classList.contains('hidden')) return;

            // Use manually selected trends if available, otherwise fallback to active AI variables so the chart isn't empty
            let varsToPlot = state.opActiveTrendsKiln.length >= 2 ? state.opActiveTrendsKiln : (window.currentAIVars || []);
            if (!varsToPlot.length) return;

            const pDims = varsToPlot.slice(0, 5).map(k => {
                const liveVals = state.opHistoryDataKiln[k] || [];
                const curr = liveVals.length ? liveVals[liveVals.length - 1].val : 0;

                let goal = curr;
                if (state.opPredictionDataKiln[k] && state.opPredictionDataKiln[k].length > 0) {
                    goal = state.opPredictionDataKiln[k][state.opPredictionDataKiln[k].length - 1]; // End of prediction
                } else {
            ...
            Plotly.react('op-dash-plotly-kiln', [{
                type: 'parcoords',
