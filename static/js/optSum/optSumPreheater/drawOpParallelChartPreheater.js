...
export function drawOpParallelChartPreheater() {
if (!document.getElementById('op-dash-plotly-preheater')) return;
            if (document.getElementById('op-dash-chart-parallel-preheater').classList.contains('hidden')) return;

            // Use manually selected trends if available, otherwise fallback to active AI variables so the chart isn't empty
            let varsToPlot = state.opActiveTrendsPreheater.length >= 2 ? state.opActiveTrendsPreheater : (window.currentAIVars || []);
            if (!varsToPlot.length) return;

            const pDims = varsToPlot.slice(0, 5).map(k => {
                const liveVals = state.opHistoryDataPreheater[k] || [];
                const curr = liveVals.length ? liveVals[liveVals.length - 1].val : 0;

                let goal = curr;
                if (state.opPredictionDataPreheater[k] && state.opPredictionDataPreheater[k].length > 0) {
                    goal = state.opPredictionDataPreheater[k][state.opPredictionDataPreheater[k].length - 1]; // End of prediction
                } else {
            ...
            Plotly.react('op-dash-plotly-preheater', [{
                type: 'parcoords',
